...
#
# class bucjet():
#     bucket_list = listtype(mdeltype(data))
# class HTte():
#     date =str
#     lskf
...
# class Metadata():
#     response=str
#     http_headers_list= listtype(modelt(HTTPHEader))


class BucketGroup(Model):
    #Response_Metadata = ListType()
    Request_Id = StringType()
    Host_Id = IntType()
    HTTP_StatusCode = IntType()
    #HTTP_Headers = IntType()
    Date = StringType()
    Bucket_name = StringType()
    Bucket_CreationDate = DateTimeType()
    Retry_Attempts = IntType()
    #Buckets_info = ListType()
    #Bucket_owner = ListType()
    DisplayName = StringType()
    ID = StringType()

class BucketInstance(Model):
    buckettGroup = ModelType(BucketGroup)
