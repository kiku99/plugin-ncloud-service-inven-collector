...
class Peering(Model):
    name = ModelType(Labels)
    network = ModelType(Labels)
    sourceVpcName = StringType()
    PeeringName = StringType()
    PeeringInstanceStatusCode = StringType(choices=('INIT','RUN','TERMTING'))
    sortedBy = StringType()
...
class Subnetwork(Model):
    ...
    region = ModelType(Labels)
    description = StringType()
    networkAclNo = StringType()
    pageNo = IntType()
    pageSize = IntType()
    subnetTypeCode = StringType(choices=('PUBLIC', 'PRIVATE'))
    subnetStatusCode = StringType(choices=('INIT', 'CREATING','RUN','TERMTING'))
...
class Gateway(Model):
    ...
    publiclp = StringType()
    natGatewayInstanceStatusCode = StringType(choices=('INIT', 'RUN','TERMTING'))
    natGatewayName = StringType()
    privateIp = StringType()
    natGatewayTypeCode = StringType(choices=('PRVT', 'PBLIP'))
    region = ModelType(Labels)



class VPCNetwork(CloudServiceResource):
    description = StringType()
    ipv4_range = StringType(deserialize_from='IPv4Range', serialize_when_none=False)
    ipv4CidrBlock = StringType()
